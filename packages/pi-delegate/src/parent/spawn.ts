...
export interface RunOptions {
  ...
  /**
   * @deprecated B4/C4: the process-wide in-flight cap is enforced OUTSIDE
   * spawnRun by the shared spawn-pool semaphore (configureSpawnPool /
   * withSpawnSlot), acquired exactly once per child by the caller:
   *   - single mode: delegate-tool.ts execute() wraps executeSingle,
   *   - parallel mode: parallel.ts wraps each runOne slot.
   * spawnRun therefore does NOT acquire a slot itself (doing so would
   * double-acquire in parallel mode and deadlock when the cap is small).
   * Kept in the interface for source-compat with callers; ignored here.
   */
 ...
 * - B4/C4: the process-wide in-flight cap is NOT acquired here — the caller wraps
 *   each child in withSpawnSlot exactly once (single mode in delegate-tool.ts,
 *   parallel mode in parallel.ts), so spawnRun stays a pure spawn primitive.
