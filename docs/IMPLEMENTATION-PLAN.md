...
> Codebase scanned: greenfield — no implementation exists yet; `nicobailon/pi-subagents` (v0.30.0) is the reference prior art for liftable primitives
...
| # | Target Capability (SPEC) | Current State | Gap | Proposed Work |
|---|---|---|---|---|
| G1 | Project is two loadable Pi extensions in a pnpm workspace: `pi-delegate` + `pi-structured-output`, peer deps pinned to `pi` 0.79.8, jiti entries (§3, header) | None | Whole scaffold absent | `pnpm-workspace.yaml` + two `package.json`; pin `pi` 0.79.8 |
| G2 | Register exactly one `delegate` tool; parent `before_agent_start` capability note (§4.1, §4.2, Appendix A) | None | Tool + injection absent | `delegate` registration, param schema, description text |
| G3 | Single-run + parallel param shapes, validation (§4.2) | None | Absent | Discriminated schema (`task` xor `parallel`), `INVALID_PARAMS` |
| G4 | Subprocess execution: binary resolution, arg/env builder, `--mode json` spawn (§3.1–§3.3) | None | Absent | Resolve `pi`; build flags/env; spawn child process |
| G5 | `--mode json` `AgentEvent` stream → `onUpdate` progress + final-message capture (§3.7, §3.8, §10) | None | Absent | Line parser; coarse progress; `agent_end`/`message_end` capture |
| G6 | Per-run temp files (`prompt.md`/`schema.json`/`output.json`), `0700`/`0600`, outside child `cwd` (§3.5) | None | Absent | Temp-dir lifecycle + cleanup on ok/error/abort |
| G7 | Agent-definition discovery (user+project scope, project-over-user) + frontmatter schema (§5) | None | Absent; frontmatter shape adoptable from pi-subagents | Glob discovery, YAML parse, validation, diagnostics |
| G8 | Model/tools/prompt resolution with §8 precedence; builtins-only ceiling via `pi.getActiveTools()` (§8.1–§8.3) | None | Absent | Precedence resolver; tool intersection/ceiling; prompt composition |
| G9 | Depth guard, default `2`, `min`-clamp down the tree (§7.1) | None | Absent; depth-env pattern adoptable | `PI_DELEGATE_DEPTH`/`_MAX_DEPTH` threading + gate |
| G10 | Explicit agent-identity cycle detection + lineage path + cap backstop (§7.2) | None | Absent; lineage-path primitive adoptable from pi-subagents `nested-path.ts` | `PI_DELEGATE_PATH` build/sanitize; cycle check |
| G11 | Capability gating by provider absence + non-forgeable per-child token (§3.4, §6) | None | Absent; gating-by-absence pattern adoptable | Token gen/blank; delegate provider arming |
| G12 | **Delegate provider** (child-side `delegate`, token-armed) (§3.4) | None | Absent | Standalone child extension |
| G13 | **Structured-output provider** (`structured_output`, schema-armed, no delegation code) (§3.4, §8.4) | None | Absent; structured-output-via-tool pattern adoptable from pi-subagents `structured-output.ts` | Standalone child extension; à-la-carte load |
| G14 | `outputSchema` hard contract: schema in, `structured_output` out, TypeBox `Compile` validation parent-side (§3.8, §8.4) | None | Absent | Schema file plumbing; `PI_OUTPUT_*` env; validate-on-read |
| G15 | `delegateAgents` immediate-target allowlist (§6) | None | Absent | Allowlist check → `TOOL_NOT_PERMITTED` |
| G16 | Deterministic preflight ordering + full error taxonomy, refusals-as-results (§4.4, §4.5) | None | Absent | Ordered checks 1–8; result-shaped errors |
| G17 | Parallel execution: ordered results, `concurrency`/`maxConcurrency`/`maxInFlightChildren`, `failFast`-on-error-only (§9) | None | Absent | Limiter; global in-flight cap; partial-tolerant results |
| G18 | Cancellation: parent `signal` → `SIGTERM`/`SIGKILL`; `failFast` sibling abort (§3.7, §9) | None | Absent | AbortController wiring; process termination |
| G19 | Untrusted child output returned as labeled tool-result, never as instruction (§10) | None | Absent | Result framing/labeling |
| G20 | Config + env precedence (`config.json`, `PI_DELEGATE_MAX_DEPTH`, `piBinaryPath`) (§11) | None | Absent | Config loader + precedence |
| G21 | Run timeout (`runTimeoutMs`) (§3.7, §10) | None | Absent | Per-run wall-clock budget |
| G22 | DX: `/delegate` status/interrupt command, `doctor`, result rendering | None | Absent | Slash command + diagnostics |
| G23 | Packaging/distribution: two packages `pi-delegate` + `pi-structured-output`, `install.mjs` (§3.4) | None | Decided (two packages) | Two publishable manifests; install script |
| G24 | Docs: README + QUICK-START (research §5) | None | Absent | Author both |
| G25 | Conformance suite mapped to §12 (1–10) | None | Absent | Test harness + cases |
| G26 | Child bash/write trust model: confirm process inheritance, default `cwd` confinement for write-capable children, optional OS-sandbox hook (research **Q5**) | None | Absent — v1 ships bash/write agents | Close Q5 spike; confinement default; sandbox config knob |
