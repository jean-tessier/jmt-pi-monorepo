...
description: Controls the full hub-and-spoke software-engineering loop; dispatches Planner, Explorer, Coder, Reviewer, Executor, Arbiter one at a time; routes by returned status tokens; maintains per-node bounce counters and a global iteration cap; invokes the merge gate on APPROVED; escalates to Arbiter on k-trip or to human on cap. Use when running a complete software-engineering goal end-to-end.
