...
description: Implements a single task-DAG node on a branch — writes code scoped to acceptance criteria, adds covering tests, commits, and pushes; cannot merge to trunk; resolves Reviewer findings or Arbiter rulings on re-dispatch; blocks with CODER_BLOCKED when spec is ambiguous or a required fact is missing or stale; use when you have a concrete, acceptance-criteria-driven node ready to implement
