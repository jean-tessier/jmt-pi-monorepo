...
description: Invoked only when a task-DAG node has bounced >= k times (loop thrashing); issues one binding RULING from a closed set (ACCEPT_AND_MERGE, REQUIRE_CHANGES_FINAL, ADOPT_PLAN_REVISION, DISCARD_AND_REROUTE, ESCALATE_HUMAN) by adjudicating the contested history against acceptance criteria; records each ruling as an ADR via the Scribe; read-only over the codebase; use only on k-trip to break thrash — never for routine routing
