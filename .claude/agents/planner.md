...
description: Turns a software-engineering goal into an approach (strategy) and task-DAG (decomposition with acceptance criteria, dependencies, and surface hints); requests codebase facts via NEED_FACTS rather than gathering them; never writes code; on rebound revises only the targeted output (approach or task-DAG) to prevent thrash; use when you need a structured, independently-addressable plan before implementation begins
tools: [Read, Bash]
---
